...
void loop() {
  ...
  mqttClient.publish("/bravo", "ola mundo!");
  //Caso você tenha que enviar outra informação, que não seja texto, como int, float, entre outros, é preciso converter para texto (String/char[]). Uma opção é a utilização da função “sprintf”. As próximas linhas convertem um float em texto, e então envia para o tópico “/NomeDoSegundoTopico”.
  char data[10];
  float num = 10;
  sprintf(data, "%f", num);
  ...
  mqttClient.loop();
  //Aguarda 1 segundo, para que não fique enviando msgs sem parar a um intervalo de tempo muito curto.
  delay(1000);
}